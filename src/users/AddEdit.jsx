...
function AddEdit({ history, match }) {
    ...
    function updateUser(id, data) {
        return userService.update(id, data)
            .then(() => {
                alertService.success('Cliente actualizado', { keepAfterRouteChange: true });
                history.push('..');
            })
            .catch(alertService.error);
    }
    ...
    useEffect(() => {
        if (!isAddMode) {
            ...
            userService.getById(id).then(user => {
                const fields = ['firstName', 'lastName', 'bornDate'];
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} onReset={reset}>
            <h1>{isAddMode ? 'Agregar Cliente' : 'Editar Cliente'}</h1>
            ...
            <div className="form-row">
                <div className="form-group col-7">
                    ...
                    <input name="bornDate" type="date" ref={register} className={`form-control ${errors.bornDate ? 'is-invalid' : ''}`} />
                    <div className="invalid-feedback">{errors.bornDate?.message}</div>
                </div>
